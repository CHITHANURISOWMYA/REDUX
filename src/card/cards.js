...
import { allCards, error } from "./action";
...
import { BackHandRounded, Margin } from "@mui/icons-material";
...
const Cards = () => {
  ...
  const redux = useSelector((state) => state.cardData.all_cards);
console.log(redux)
  ...
  useEffect(() => {
    axios
      ...
      .then((response) => dispatch(allCards(response.data.results)));
  }, []);
              ...
              <CardHeader
                    avatar={
                      <Avatar sx={{ bgcolor: red[500] }} aria-label="recipe">
                        <img src={val.userData.profilePic} />
                      </Avatar>
                    }
                    action={
                    ...
                    title={val.pageData ? val.pageData.fullName : "Dr Shindhu"}
                    subheader={val.userData.caption}
                  />
                  <CardContent sx={{ width: 230, whiteSpace: 'nowrap',height:"3%" }}>
                    <Typography fontWeight="500" sx={{ textOverflow:"ellipsis",overflow: 'hidden' }}  >
                      {val.articleData
                        ? val.articleData.articleDescription
                        : "  "}
                    </Typography>
                  </CardContent>
                  <Box component="div" className="content" sx={{ bgcolor: ""}}>
                    <CardMedia
                      component="img"
                      height="194"
                      image={
                        val.eventData
                          ? val.eventData.eventImage
                    ...
                    {val.articleData ? <Box sx={{padding:2}}>{val.articleData.articleTitle}
                    <Button sx={{border:"1px solid black",margin:5}}>
                      Read Article
                    </Button>
                    </Box>:
                    <Box sx={{padding:1}}>
                      <Typography variant="span" component="span">Event</Typography>
                      <Button sx={{ml:"100px",border:"1px solid black",borderRadius:"10px",mt:3}}>Register</Button>
                    
                    <Typography sx={{mt:3,padding:2}}>
                      Free Health camp for Family, Woman and Child health
                    </Typography>
                  ...
                  <Box sx={{padding:2}}>{count} persons likes</Box>
